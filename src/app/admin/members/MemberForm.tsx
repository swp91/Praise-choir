...
export function MemberForm({ member, sections, instruments, action, title, submitLabel, cancelHref }: Props) {
  return (
    <form action={action} encType="multipart/form-data" className="border border-line bg-card">
      {member ? <input type="hidden" name="id" value={member.id} /> : null}
      {member?.photoAssetId ? <input type="hidden" name="existing_photo_asset_id" value={member.photoAssetId} /> : null}
        ...
        <div className="min-[760px]:col-span-2">
          ...
          <div className="flex flex-col gap-4 border border-line bg-cream p-4 min-[560px]:flex-row min-[560px]:items-center">
            {member?.photoUrl ? (
              ...
              <div className="flex h-24 w-24 items-center justify-center rounded-full bg-line font-ko text-[28px] text-ink-mute">
            ...
            <div className="flex-1">
              ...
              <input id="photo_file" name="photo_file" type="file" accept="image/*" className="sr-only" />
            </div>
          </div>
        </div>
      </div>
