...
function MusicStaffForm({ staff }: { staff: AdminMusicStaff }) {
  return (
    <form action={updateMusicStaffAction} encType="multipart/form-data" className="border border-line bg-card">
      ...
      <div className="grid gap-4 px-5 py-5 min-[760px]:grid-cols-2">
        ...
        <div>
          <label className={labelClass} htmlFor={`role-en-${staff.id}`}>영문 직무</label>
          <input id={`role-en-${staff.id}`} name="role_en" className={inputClass} defaultValue={staff.roleEn} />
        </div>
        <div>
        ...
        <label className="flex items-end gap-2 pb-2 font-ko text-[13px] text-ink">
...
function PeopleSelect({
  ...
  return (
    <select name="person_id" className={inputClass} defaultValue={defaultValue ?? ''}>
      <option value="">대원 선택</option>
      {people.map((person) => (
        <option key={person.id} value={person.id}>
          {person.label} · {person.sectionName}
        </option>
      ))}
    </select>
  );
...
function AddOfficerForm({ people }: { people: AdminLeaderPersonOption[] }) {
  return (
    <form action={createOfficerAction} className="grid gap-3 border-b border-line bg-card-head px-5 py-4 min-[900px]:grid-cols-[1.2fr_1fr_1fr_90px_auto]">
      <PeopleSelect people={people} />
      <input name="role_text" className={inputClass} placeholder="직무 예: 총무" />
      <input name="role_en" className={inputClass} placeholder="영문 직무 선택 입력" />
      <input name="sort_order" type="number" className={inputClass} placeholder="순서" />
      ...
      <button
        type="submit"
        className="border border-gold-deep bg-gold-deep px-4 py-2.5 font-ko text-[13px] font-bold text-cream transition hover:bg-ink min-[900px]:col-start-5"
      >
        + 임원 추가
      </button>
    </form>
...
function OfficerRow({ officer, people }: { officer: AdminOfficer; people: AdminLeaderPersonOption[] }) {
  return (
    <tr className="border-b border-line-soft last:border-b-0">
      ...
      <td className="px-4 py-3">
        <form id={`officer-${officer.id}`} action={updateOfficerAction}>
          <input type="hidden" name="id" value={officer.id} />
          <PeopleSelect people={people} defaultValue={officer.personId} />
        </form>
      </td>
      <td className="px-4 py-3">
        <input form={`officer-${officer.id}`} name="role_text" className={inputClass} defaultValue={officer.roleText} />
      </td>
      <td className="px-4 py-3">
        <input form={`officer-${officer.id}`} name="role_en" className={inputClass} defaultValue={officer.roleEn} />
      </td>
      <td className="px-4 py-3">
        <input
          form={`officer-${officer.id}`}
          name="sort_order"
          type="number"
          className={inputClass}
          defaultValue={officer.sortOrder}
        />
      </td>
      <td className="px-4 py-3">
        <label className="flex items-center gap-2 font-ko text-[13px] text-ink">
          <input form={`officer-${officer.id}`} type="checkbox" name="is_active" defaultChecked={officer.isActive} />
          공개
        </label>
      </td>
      <td className="px-4 py-3">
        <div className="flex gap-2">
          ...
          <form action={deleteOfficerAction}>
            ...
            <button
              type="submit"
              className="border border-red-200 bg-red-50 px-3 py-2 font-ko text-[12px] text-red-700 transition hover:border-red-500 hover:bg-red-100"
            >
              해제
            </button>
          </form>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 space-y-6">
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 min-[1000px]:grid-cols-3">
              {data.musicStaff.map((staff) => (
                <MusicStaffForm key={staff.id} staff={staff} />
              ))}
            </div>
          </section>
          ...
          <section className="border border-line bg-card">
            ...
            <AddOfficerForm people={data.people} />
              ...
              <table className="w-full min-w-[1100px] border-collapse">
                <thead>
                  <tr className="border-b border-line-soft bg-cream/70 text-left">
                    ...
                    <th className="px-4 py-3 font-ko text-[12px] text-ink-mute">대원 선택</th>
                    <th className="px-4 py-3 font-ko text-[12px] text-ink-mute">직무</th>
                    <th className="px-4 py-3 font-ko text-[12px] text-ink-mute">영문 직무</th>
                ...
                <tbody>
                  {data.officers.map((officer) => (
                    <OfficerRow key={officer.id} officer={officer} people={data.people} />
                  ))}
