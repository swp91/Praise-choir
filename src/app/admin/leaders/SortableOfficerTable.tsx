...
export default function SortableOfficerTable({ officers: initialOfficers, editId, actions }: Props) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const reordered = arrayMove(officers, oldIndex, newIndex).map((officer, index) => ({
      ...officer,
      sortOrder: index,
    }));
    ...
    setSaving(true);
    try {
      await actions.reorder(reordered.map((officer) => officer.id));
    } finally {
      setSaving(false);
    }
  }
          ...
          <table className="w-full min-w-[820px] border-collapse">
            ...
            <tbody>
              {officers.map((officer) => (
                <OfficerRow key={officer.id} officer={officer} editId={editId} actions={actions} />
              ))}
              {!officers.length ? (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center font-ko text-[13px] text-ink-soft">
                    등록된 임원진이 없습니다.
                  </td>
                </tr>
              ) : null}
