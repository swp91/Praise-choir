...
export default function SortableMusicStaffTable({ staff: initialStaff, editId, actions }: Props) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const reordered = arrayMove(staff, oldIndex, newIndex).map((item, index) => ({
      ...item,
      sortOrder: index,
    }));
      ...
      <SortableContext items={staff.map((item) => item.id)} strategy={verticalListSortingStrategy}>
        <div className="relative overflow-x-auto">
          ...
          <table className="w-full min-w-[760px] border-collapse table-fixed">
            ...
            <tbody>
              {staff.map((item) => (
                <StaffRow key={item.id} staff={item} editId={editId} actions={actions} />
              ))}
              {!staff.length ? (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center font-ko text-[13px] text-ink-soft">
                    스태프 섹션에 등록된 대원이 없습니다.
                  </td>
                </tr>
              ) : null}
            </tbody>
