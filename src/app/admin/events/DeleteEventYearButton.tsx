...
import { deleteEventYearAction } from './actions';
...
export default function DeleteEventYearButton({ year, eventCount }: Props) {
      ...
      {open ? (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-ink/55 px-5">
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="delete-event-year-title"
          ...
          >
            <div className="border-b border-line bg-card-head px-5 py-4">
              <h2 id="delete-event-year-title" className="font-ko text-[18px] font-bold text-ink">
                {year}년을 삭제할까요?
              </h2>
            </div>
            <div className="px-5 py-5">
              <p className="font-ko text-[14px] leading-relaxed text-ink-soft">
                이 년도와 연결된 일정 {eventCount}개가 함께 삭제됩니다. 삭제된 데이터는 복구할 수 없습니다.
              </p>
              <div className="mt-6 flex justify-end gap-2">
                ...
                <form action={deleteEventYearAction}>
